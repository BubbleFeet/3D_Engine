...
void ConstantBuffer::Initialize(uint32_t bufferSize, const void* initData)
{
	...
	Graphics::GraphicsSystem::Get()->GetDevice()->CreateBuffer(&desc, subresourceDataPtr, &mBuffer);
}
...
{
	Graphics::GraphicsSystem::Get()->GetContext()->UpdateSubresource(mBuffer, 0, nullptr, data, 0, 0);
}
...
{
	Graphics::GraphicsSystem::Get()->GetContext()->VSSetConstantBuffers(slot, 1, &mBuffer);
}
...
{
	Graphics::GraphicsSystem::Get()->GetContext()->GSSetConstantBuffers(slot, 1, &mBuffer);
}
...
{
	Graphics::GraphicsSystem::Get()->GetContext()->PSSetConstantBuffers(slot, 1, &mBuffer);
}
...
{
	Graphics::GraphicsSystem::Get()->GetContext()->CSSetConstantBuffers(slot, 1, &mBuffer);
}